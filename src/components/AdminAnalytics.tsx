...
export default function AdminAnalytics({ password }: { password: string }) {
  ...
  if (loading) {
    return (
      <div className="mb-6 bg-white rounded-2xl shadow-sm p-6">
        <div className="text-center py-8 text-neutral">Загрузка аналитики...</div>
      </div>
    );
  }
  ...
  return (
    <div className="mb-6">
      ...
      <button
        ...
        className="w-full flex items-center justify-between bg-white rounded-2xl shadow-sm p-4 mb-4 hover:shadow-md transition-shadow"
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <div className="font-semibold text-dark">Аналитика</div>
            <div className="text-xs text-neutral">Последние 30 дней</div>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <div className="hidden sm:flex items-center gap-6 text-sm">
            <div><span className="text-neutral">Просмотры:</span> <strong className="text-dark">{data.totalViews}</strong></div>
            <div><span className="text-neutral">Заявки:</span> <strong className="text-dark">{data.totalRequests}</strong></div>
            <div><span className="text-neutral">Конверсия:</span> <strong className="text-dark">{data.conversionRate}%</strong></div>
          </div>
          <svg className={`w-5 h-5 text-neutral transition-transform ${expanded ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {expanded && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
            {[
              { label: "Просмотры", value: data.totalViews, color: "bg-blue-50", iconColor: "text-blue-600" },
              { label: "Заявки", value: data.totalRequests, color: "bg-green-50", iconColor: "text-green-600" },
              { label: "Конверсия", value: `${data.conversionRate}%`, color: "bg-yellow-50", iconColor: "text-yellow-600" },
              { label: "Выручка", value: `${data.totalRevenue.toLocaleString("ru-RU")} ₽`, color: "bg-purple-50", iconColor: "text-purple-600" },
              { label: "Пользователи", value: `${data.newUsers} / ${data.totalUsers}`, color: "bg-pink-50", iconColor: "text-pink-600" },
            ].map((card) => (
