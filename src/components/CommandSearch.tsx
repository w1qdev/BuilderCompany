...
function CompassIcon({ className }: { className?: string }) {
  return (
    <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 2a10 10 0 100 20 10 10 0 000-20zm0 0v0m3.5 6.5l-2 5-5 2 2-5 5-2z" />
    </svg>
  );
}
...
function FlaskIcon({ className }: { className?: string }) {
  return (
    <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 3h6m-5 0v5.172a2 2 0 01-.586 1.414l-3.828 3.828A3 3 0 007.708 19h8.584a3 3 0 002.122-5.121l-3.828-3.828A2 2 0 0114 8.637V3" />
    </svg>
  );
}
...
export default function CommandSearch({ isOpen, onClose }: CommandSearchProps) {
  ...
  if (results) {
    for (const p of results.pages) flatItems.push({ kind: "page", data: p });
    for (const e of results.equipment) flatItems.push({ kind: "equipment", data: e });
    for (const r of results.requests) flatItems.push({ kind: "request", data: r });
  }
  ...
  const hasResults = results && (results.pages.length > 0 || results.equipment.length > 0 || results.requests.length > 0);
              ...
              <div ref={listRef} className="max-h-[50vh] overflow-y-auto">
                ...
                {!query.trim() && (
                  <div className="px-4 py-10 text-center text-sm text-gray-400 dark:text-white/40">
                    ...
                    <p>Нажмите <kbd className="px-1.5 py-0.5 text-[10px] font-medium bg-gray-100 dark:bg-white/5 rounded border border-gray-200 dark:border-white/10">Ctrl+K</kbd> для быстрого поиска</p>
                  </div>
                )}
                    ...
                    {results.pages.map((page) => {
                      ...
                      return (
                        <button
                          key={page.href}
                          data-search-item
                          onClick={() => navigateToItem({ kind: "page", data: page })}
                        ...
                        >
                          ...
                          <span className="text-sm font-medium text-dark dark:text-white truncate">{page.title}</span>
                        </button>
                      );
                    })}
                  </div>
                    ...
                    {results.equipment.map((eq) => {
                      ...
                      return (
                        <button
                          key={eq.id}
                          data-search-item
                          onClick={() => navigateToItem({ kind: "equipment", data: eq })}
                          className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-colors ${
                            selectedIndex === idx
                              ? "bg-primary/5 dark:bg-primary/10"
                              : "hover:bg-gray-50 dark:hover:bg-white/5"
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="text-sm font-medium text-dark dark:text-white truncate">{eq.name}</div>
                            <div className="text-xs text-gray-400 dark:text-white/40 truncate">
                              {[eq.type, eq.serialNumber].filter(Boolean).join(" \u00b7 ") || "\u2014"}
                            </div>
                          </div>
                    ...
                    {results.requests.map((req) => {
                      ...
                      return (
                        <button
                          key={req.id}
                          data-search-item
                          onClick={() => navigateToItem({ kind: "request", data: req })}
                          className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-colors ${
                            selectedIndex === idx
                              ? "bg-primary/5 dark:bg-primary/10"
                              : "hover:bg-gray-50 dark:hover:bg-white/5"
              ...
              {hasResults && (
                <div className="flex items-center gap-4 px-4 py-2.5 border-t border-gray-200 dark:border-white/10 text-[11px] text-gray-400 dark:text-white/30">
                  <span className="flex items-center gap-1">
                    <kbd className="px-1 py-0.5 bg-gray-100 dark:bg-white/5 rounded border border-gray-200 dark:border-white/10 font-mono">&uarr;</kbd>
                    <kbd className="px-1 py-0.5 bg-gray-100 dark:bg-white/5 rounded border border-gray-200 dark:border-white/10 font-mono">&darr;</kbd>
                    навигация
                  </span>
                  <span className="flex items-center gap-1">
                    <kbd className="px-1 py-0.5 bg-gray-100 dark:bg-white/5 rounded border border-gray-200 dark:border-white/10 font-mono">Enter</kbd>
                    перейти
                  </span>
                </div>
              )}
