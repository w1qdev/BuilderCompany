      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        >
          ...
          <p className="text-neutral dark:text-white/60 mt-3 max-w-2xl mx-auto text-lg">
            Выберите тип услуги и сроки выполнения работ. Мы свяжемся с вами для
            уточнения деталей
          </p>
        </motion.div>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {serviceTypes.map((s, index) => (
              <motion.button
                ...
                className={`group relative overflow-hidden rounded-2xl p-6 text-left transition-[box-shadow,transform,ring-color] duration-300 ${
                  service === s.id
              ...
              >
                ...
                <div
                  className={`absolute inset-0 bg-gradient-to-br ${s.gradient} opacity-0 ${service === s.id ? "opacity-10" : "group-hover:opacity-5"} transition-opacity`}
                />

                {/* Icon */}
                <div
                  className={`relative w-14 h-14 rounded-xl bg-gradient-to-br ${s.gradient} flex items-center justify-center mb-4 shadow-lg ${service === s.id ? "scale-110" : "group-hover:scale-110"} transition-transform`}
                >
                  <svg
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {urgencyOptions.map((u, index) => (
              <motion.button
                ...
                className={`group relative overflow-hidden rounded-2xl p-8 text-left transition-[box-shadow,transform,ring-color] duration-300 ${
                  urgency === u.id
              ...
              >
                ...
                <div>
                  <h4
                    className={`text-xl font-bold mb-2 ${urgency === u.id ? "text-white" : "text-dark dark:text-white"}`}
                  >
                    {u.label}
                  </h4>
                  <p
                    className={`text-lg font-semibold ${urgency === u.id ? "text-primary" : "text-neutral dark:text-white/60"}`}
                  >
                    {u.duration}
                  </p>
                </div>
                  ...
                  <div className="flex items-start gap-3 mb-4">
                    ...
                    <div className="text-sm text-dark dark:text-white">
                      ...
                      <ul className="space-y-1 text-neutral dark:text-white/70">
                        ...
                        <li>
                          • Отправим вам документы на почту в течении суток
                        </li>
