    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          <div
            key={s.label}
            className="bg-white rounded-lg border border-gray-100 p-2.5 flex items-center gap-2 hover:shadow-sm transition-shadow cursor-default"
          >
            <div
      ...
      <div className="flex items-center gap-2">
        ...
        <div
          className="h-7 px-2.5 rounded-lg text-[10px] font-medium flex items-center gap-1 cursor-pointer hover:opacity-90 transition-opacity"
          style={{ backgroundColor: "#E87A2E", color: "#fff" }}
        >
          <svg
            className="w-3 h-3"
            fill="none"
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        {data.rows.map((row, i) => (
          <div
            ...
            className={`grid grid-cols-[16px_1fr_auto_auto_auto] gap-2 px-3 py-2.5 text-xs text-gray-600 items-center cursor-pointer transition-colors ${
              ...
              selectedRows.has(i) ? "bg-orange-50/60" : ""
            }`}
          ...
          >
            ...
            <span
              className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium w-fit ${
                statusMap[row[3]]?.class
              }`}
            >
              <span
                className={`w-1.5 h-1.5 rounded-full ${statusMap[row[3]]?.dot}`}
              />
        ...
        <div className="px-3 py-2 bg-gray-50/50 border-t border-gray-100 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <div className="w-5 h-5 rounded bg-gray-100 flex items-center justify-center text-[9px] text-gray-400 cursor-pointer hover:bg-gray-200 transition-colors">
              &lt;
            </div>
            <div
              className="w-5 h-5 rounded flex items-center justify-center text-[9px] text-white font-bold cursor-pointer"
              style={{ backgroundColor: "#E87A2E" }}
            >
              1
            </div>
            <div className="w-5 h-5 rounded bg-gray-100 flex items-center justify-center text-[9px] text-gray-500 cursor-pointer hover:bg-gray-200 transition-colors">
              2
            </div>
            <div className="w-5 h-5 rounded bg-gray-100 flex items-center justify-center text-[9px] text-gray-500 cursor-pointer hover:bg-gray-200 transition-colors">
              3
...
function SchedulePreview({
  ...
  data: {
    months: string[];
    items: { name: string; month: number; color: string }[];
  };
}) {
  const [hoveredItem, setHoveredItem] = useState<number | null>(null);
  const [tooltip, setTooltip] = useState<{ row: number; col: number } | null>(
    null
  );
  return (
    <div className="space-y-3">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="flex items-center border-b border-gray-100">
          <div className="w-36 shrink-0 px-3 py-2 text-[10px] font-semibold text-gray-400 uppercase">
            Оборудование
          </div>
          <div className="flex-1 grid grid-cols-6 gap-px">
            {data.months.map((m) => (
              <div
                key={m}
                className="text-center py-2 text-[10px] font-medium text-gray-400 bg-gray-50/50"
              >
                {m}
              </div>
            ))}
          ...
          >
            <div className="w-36 shrink-0 px-3 py-2.5">
              ...
              <div className="text-[9px] text-gray-400">
                Зав. № {["А-1234", "F-5678", "W-9012", "T-3456"][i]}
              </div>
            </div>
            <div className="flex-1 grid grid-cols-6 gap-px">
              {data.months.map((_, mi) => (
                <div
                  ...
                  onMouseEnter={() =>
                    mi === item.month && setTooltip({ row: i, col: mi })
                  }
...
function RequestsPreview({
  ...
  data: {
    requests: { id: string; service: string; status: string; color: string }[];
  };
}) {
  ...
  const extraInfo: Record<
    string,
    { date: string; items: number; manager: string }
  > = {
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          <div
            key={s.label}
            className="bg-white rounded-lg border border-gray-100 p-2 text-center hover:shadow-sm transition-shadow cursor-default"
          >
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <span
                className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${r.color}`}
              >
                {r.status}
              </span>
            </div>
                ...
                <div key={step} className="flex items-center gap-1 flex-1">
                  <div
                    className={`w-4 h-4 rounded-full flex items-center justify-center text-[7px] font-bold ${
                      si < current ? "text-white" : "bg-gray-100 text-gray-400"
                    }`}
                  ...
                  {si < steps.length - 1 && (
                    <div
                      className={`flex-1 h-px ${
                        si < current - 1 ? "bg-[#E87A2E]" : "bg-gray-100"
                      }`}
                    />
                  )}
                </div>
...
function AnalyticsPreview({
  ...
  data: { bars: number[]; labels: string[] };
}) {
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-4 gap-2">
        {[
          ...
          <div
            key={s.label}
            className="bg-white rounded-lg border border-gray-100 p-2 hover:shadow-sm transition-shadow cursor-default"
          >
            <div className="flex items-center justify-between mb-1">
              ...
              >
                <svg
                  className={`w-2 h-2 ${s.up ? "" : "rotate-180"}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
          ...
          {data.bars.map((v, i) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className="w-full flex items-end gap-px justify-center"
...
function ThemesPreview({
  ...
  data: {
    themes: { name: string; bg: string; primary: string; card: string }[];
  };
}) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div
        ...
        style={{ backgroundColor: t.card, borderColor: `${t.primary}20` }}
      >
        <div
        ...
        >
          <div
            className="absolute -bottom-4 left-3 w-9 h-9 rounded-full border-2 flex items-center justify-center text-[10px] font-bold"
            style={{
        ...
        <div className="pt-5 px-3 pb-2.5">
          <div
            ...
            style={{
              color:
                t.bg.startsWith("#0") || t.bg.startsWith("#1")
                  ? "#E5E5E5"
                  : "#1F2937",
            }}
          >
        ...
        {data.themes.map((theme, i) => (
          ...
          >
            ...
            <div className="p-1.5" style={{ backgroundColor: theme.bg }}>
              <div className="flex gap-1">
                ...
                <div className="flex-1 space-y-1">
                  ...
                  >
                    <div
                      className="h-1 w-3/4 rounded-full mt-1 mx-1"
            ...
            <div
              ...
              style={{
                backgroundColor:
                  theme.card.includes("#FFF") || theme.card === "#FFFFFF"
                    ? "#FAFAFA"
                    : theme.card,
              }}
            >
      ...
      <div className="flex items-center gap-3">
        ...
        >
          ...
          <div
            ...
            style={compact ? { backgroundColor: t.primary } : {}}
          >
            <div
              className={`w-2.5 h-2.5 rounded-full bg-white shadow-sm absolute top-0.5 transition-all ${
                compact ? "left-[calc(100%-14px)]" : "left-0.5"
              }`}
            />
          </div>
        </div>
...
export default function DashboardShowcase() {
  ...
  return (
    <section
      className="py-20 bg-warm-bg dark:bg-dark relative overflow-hidden"
      id="dashboard"
    >
      <div className="max-w-8xl mx-auto px-4 sm:px-6">
        ...
        >
          ...
          <p className="text-neutral dark:text-white/60 mt-3 max-w-2xl mx-auto">
            Управляйте средствами измерений, следите за сроками поверки и
            подавайте заявки — всё в одном месте
          </p>
        </motion.div>
            ...
            <div className="bg-gray-50 dark:bg-dark-light rounded-2xl p-6 sm:p-8 border border-gray-200 dark:border-white/10 min-h-[400px]">
              ...
              <div onClick={handlePreviewInteraction}>
                <AnimatePresence mode="wait">
                  ...
                  >
                    {PreviewComponent && (
                      <PreviewComponent data={feature.preview} />
                    )}
                  </motion.div>
                </AnimatePresence>
              </div>
