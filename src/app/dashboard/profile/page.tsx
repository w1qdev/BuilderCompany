...
export default function ProfilePage() {
  ...
  const uploadImage = async (file: File, type: "avatar" | "cover") => {
    ...
    try {
      ...
      setUser((u) =>
        u ? { ...u, [type === "cover" ? "coverImage" : "avatar"]: data.url } : u
      );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setCompanyForm({ ...companyForm, position: e.target.value })
            }
            className={inputClass}
            placeholder="Главный метролог"
          />
        </div>
        <div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setCompanyForm({ ...companyForm, kpp: e.target.value })
                }
                className={inputClass}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setCompanyForm({ ...companyForm, legalAddress: e.target.value })
              }
              className={inputClass}
              ...
              <div className="px-2 pt-2 space-y-1.5">
                <div className="flex gap-1.5">
                  <div
                    ...
                    style={{ backgroundColor: t.preview.primary, opacity: 0.8 }}
                  />
                  <div
                    className="h-2 rounded-full w-4"
                    style={{ backgroundColor: t.preview.text, opacity: 0.15 }}
                  />
                </div>
                <div className="flex gap-1.5">
                  <div
                ...
                <div
                  ...
                  style={{ backgroundColor: t.preview.text, opacity: 0.12 }}
                />
              </div>
              {active && (
            ...
            <div className="px-2 py-2 bg-white dark:bg-dark-light">
              <span
                className={`text-xs font-medium block truncate ${
                  active ? "text-primary" : "text-gray-900 dark:text-white/70"
                }`}
              >
                {t.name}
              </span>
      ...
      <div className="border-t border-gray-100 dark:border-white/10 pt-6 space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-white/40 mb-3">
            Изменяет базовый размер текста в дашборде
          </p>
          <FontSizePicker />
        </div>
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-white/40 mb-3">
            Меньше отступов — больше данных на экране
          </p>
          <CompactToggle />
        </div>
      </div>
  ...
  return (
    <button
      ...
      className={`relative w-11 h-6 rounded-full transition-colors ${
        compact ? "bg-primary" : "bg-gray-300 dark:bg-white/20"
      }`}
    >
      <span
        className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${
          compact ? "translate-x-5" : ""
        }`}
      />
    </button>
  );
          ...
          <div className="bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-800/30 rounded-xl p-5 space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="py-2 px-4 rounded-lg text-sm font-medium border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
              >
                Отмена
              </button>
            </div>
...
function NotificationsTab() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-base font-semibold text-gray-900 dark:text-white mb-1">
          Напоминания о поверке
        </h3>
        <p className="text-sm text-gray-500 dark:text-white/40 mb-4">
          За сколько дней до поверки напоминать
        </p>
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-white/40 mb-3">
          Привяжите Telegram для получения уведомлений. Напишите нашему боту,
        ...
        {maxLinked ? (
          ...
          <button
            onClick={async () => {
              setMaxLoading(true);
              try {
                const res = await fetch("/api/user/max-link", {
                  method: "POST",
                });
                const data = await res.json();
                if (data.code) {
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {sessions.map((s) => (
            <div
              key={s.id}
              className="flex items-center justify-between px-4 py-3 rounded-xl bg-gray-50 dark:bg-white/5"
            >
      ...
      {loading ? (
        ...
        <div className="space-y-1.5">
          {logs.map((log) => (
            <div
              key={log.id}
              className="flex items-center justify-between px-4 py-2.5 rounded-xl bg-gray-50 dark:bg-white/5 text-sm"
            >
              <div className="flex items-center gap-3 min-w-0">
                <svg
                  className="w-4 h-4 shrink-0 text-gray-500 dark:text-white/30"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <span className="text-gray-900 dark:text-white/70 truncate">
        ...
        <div className="bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800/30 rounded-xl p-5 space-y-4">
          ...
          <div className="flex gap-3">
            <button
              ...
              disabled={deleting}
              className="inline-flex items-center gap-1.5 bg-red-600 text-white py-2 px-4 rounded-lg text-sm font-medium hover:bg-red-700 transition-colors disabled:opacity-50"
            >
              {deleting ? (
                "Удаление..."
              ) : (
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 py-2 px-4 rounded-lg text-sm font-medium border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
            >
              <svg
                className="w-4 h-4"
                fill="none"
